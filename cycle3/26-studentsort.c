...
int partition(Student* st, int p, int r){
    Student temp;
    float x = st[r].height;
    int i = p-1;
    for(int j = p; j < r; j++)
...
void quicksort(Student* st, int p, int r){
    if(p < r){
...
void merge(Student* st1, int p, int q, int r){
    int n1 = q - p + 1;
    int n2 = r - q;
    Student L[n1], R[n2];
    for(int i = 0; i < n1; i++)
...
void mergeSort(Student* st1, int p, int r){
    if(p < r){
        int q = floor((p+r)/2);
        mergeSort(st1, p, q);
        mergeSort(st1, q+1, r);
...
void main(){
    ...
    FILE *fpq = fopen("qsort_student.txt", "w");
    FILE *fpm = fopen("msort_student.txt", "w");
